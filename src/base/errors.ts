...
export type WithSpan<T> = { span: SourceSpan; value: T };

export function trimSpan(data: WithSpan<string>): WithSpan<string> {
...
export function mergeSpan(...spans: WithSpan<string>[]): WithSpan<string> {
  return {
...
export function formatSpan(content: string, span: SourceSpan): string {
  const before = content.slice(0, span.start).split("\n");
  const main = content.slice(span.start, span.end).split("\n");
  const after = content.slice(span.end).split("\n");

  const startLineNo = before.length;
  const startColNo = before[startLineNo - 1].length;

  return `${startLineNo}번째 줄 ${startColNo}번째 글자
...
class ChaltteokError {
  ...
  constructor(message: string, public traceback: [SourceFile, SourceSpan][]) {
    this.message = `${traceback
      .map(([file, span]) => `${file.path}, ${formatSpan(file.content, span)}`)
      .join("\n")}
${josa(message)}`.trim();
  }
}
