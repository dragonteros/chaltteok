...
const grammar: Grammar = {
  ...
  ParserRules: [
    ...
    { name: "program$ebnf$2", symbols: [], postprocess: () => null },
    {
      ...
      postprocess: ([stmts]) => filter(stmts),
    },
    ...
    { name: "statement", symbols: ["funDef"], postprocess: id },
    {
      name: "statement",
      symbols: ["expr"],
      postprocess: ([expr]) => intoExpr(expr),
    },
    {
      name: "vocabDef$subexpression$1",
      symbols: [tokenizer.has("NewLine") ? { type: "NewLine" } : NewLine],
    },
    {
      name: "vocabDef$subexpression$1",
      symbols: [
    ...
    {
      name: "vocabDef",
      symbols: ["vocabCore", "vocabDef$subexpression$1"],
      postprocess: ([vocab]) => ({ type: "VocabDef", vocab }),
    },
    {
      name: "synonymDef$subexpression$1",
      symbols: [tokenizer.has("NewLine") ? { type: "NewLine" } : NewLine],
    },
    {
      name: "synonymDef$subexpression$1",
    ...
    {
      name: "vocabFunDef$ebnf$1",
      symbols: ["vocabFunDef$ebnf$1", "spaceOrNewLine"],
      postprocess: (d) => d[0].concat([d[1]]),
    },
    {
      name: "vocabFunDef",
      symbols: ["vocabCore", { literal: ":" }, "vocabFunDef$ebnf$1", "body"],
      postprocess: ([vocab, , , body]) => ({
        type: "VocabFunDef",
        vocab,
        body,
      }),
    ...
    {
      name: "pattern$ebnf$1",
      symbols: ["pattern$ebnf$1", "spaceOrNewLine"],
      postprocess: (d) => d[0].concat([d[1]]),
    },
    {
      name: "pattern",
      symbols: ["text_", { literal: ":" }, "pattern$ebnf$1"],
      postprocess: id,
    },
    ...
    {
      name: "body",
      symbols: ["exprBody", "body$ebnf$1", "doubleNewLines"],
      postprocess: ([expr]) => intoExpr(expr),
    },
    {
      name: "body",
      symbols: [tokenizer.has("JS") ? { type: "JS" } : JS],
      postprocess: ([block]) => ({ type: "JSBody", block: merge(block) }), // cast
    },
    { name: "exprBody", symbols: ["expr"], postprocess: ([expr]) => expr },
    {
      name: "exprBody",
      symbols: ["exprBody", "maybeNewLine", "expr"],
      postprocess: (exprs) => merge(...exprs),
    },
    {
      name: "doubleNewLines",
      symbols: [
    ...
    {
      name: "doubleNewLines$ebnf$1",
      symbols: [
        "doubleNewLines$ebnf$1",
        tokenizer.has("Whitespace") ? { type: "Whitespace" } : Whitespace,
      ],
      postprocess: (d) => d[0].concat([d[1]]),
    },
    {
      name: "doubleNewLines$subexpression$1",
      symbols: [tokenizer.has("NewLine") ? { type: "NewLine" } : NewLine],
    },
    {
      name: "doubleNewLines$subexpression$1",
    ...
    },
    {
      name: "doubleNewLines",
      symbols: [
        tokenizer.has("NewLine") ? { type: "NewLine" } : NewLine,
        "doubleNewLines$ebnf$1",
        "doubleNewLines$subexpression$1",
      ],
      postprocess: skip,
    },
    { name: "expr$ebnf$1", symbols: [] },
    {
      ...
      symbols: [tokenizer.has("Word") ? { type: "Word" } : Word],
      postprocess: id,
    },
    {
      name: "expr$ebnf$1$subexpression$1",
    ...
    {
      name: "expr$ebnf$1",
      symbols: ["expr$ebnf$1", "expr$ebnf$1$subexpression$1"],
      postprocess: (d) => d[0].concat([d[1]]),
    },
    {
      name: "expr",
      symbols: [
        tokenizer.has("Word") ? { type: "Word" } : Word,
        "expr$ebnf$1",
        tokenizer.has("SentenceFinal")
          ? { type: "SentenceFinal" }
          : SentenceFinal,
      ],
      postprocess: ([text, texts, final]) => merge(text, ...texts, final),
    },
    {
      name: "spaceOrNewLine",
      symbols: [
    ...
    {
      name: "maybeNewLine",
      symbols: ["maybeNewLine$ebnf$1"],
      postprocess: ([spaces]) => merge(...spaces),
    },
    { name: "maybeNewLine$ebnf$2", symbols: [] },
    {
      name: "maybeNewLine$ebnf$2",
      symbols: [
        "maybeNewLine$ebnf$2",
        tokenizer.has("Whitespace") ? { type: "Whitespace" } : Whitespace,
      ],
    ...
    },
    {
      ...
      postprocess: ([leading, newline, trailing]) =>
    ...
    {
      name: "_text_",
      symbols: ["_text_$ebnf$1", "text_"],
      postprocess: ([, text]) => text,
    },
    {
      name: "text_",
      symbols: [tokenizer.has("Word") ? { type: "Word" } : Word, "maybeText"],
      postprocess: ([text, maybeText]) =>
        trimStringWithMetadata(merge(text, maybeText)),
    },
    { name: "maybeText$ebnf$1", symbols: [] },
    {
      name: "maybeText$ebnf$1$subexpression$1",
      symbols: [tokenizer.has("Word") ? { type: "Word" } : Word],
      postprocess: id,
    },
    {
    ...
    {
      name: "maybeText$ebnf$1",
      symbols: ["maybeText$ebnf$1", "maybeText$ebnf$1$subexpression$1"],
      postprocess: (d) => d[0].concat([d[1]]),
    },
    {
      name: "maybeText",
      symbols: ["maybeText$ebnf$1"],
      postprocess: function ([texts]) {
        texts = filter(texts);
